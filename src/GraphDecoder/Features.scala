...
class Features(options: Map[Symbol, String], private var myFeatureNames: List[String], labelSet: Array[String]) {
  ...
  def ffFragHead {
    ...
    addFeature("C1NotFragHead", if (node1.concept != graph.spans(node1.spans(0)).amr.concept) {
      1.0
    } else {
      0.0
    }, 0.0)
    addFeature("C2NotFragHead", if (node2.concept != graph.spans(node2.spans(0)).amr.concept) {
      1.0
    } else {
      0.0
    }, 0.0)
  ...
  //    }
  def dependencySpan(node: Node): IndexedSeq[Int] = {
    // Returns the node's span in the dependency parse
    //logger(1, "node.spans = "+node.spans)
    //logger(1, "node.spans(0) = "+node.spans(0).toString)
    //logger(1,"span = "+(graph.spans(node.spans(0)).start, graph.spans(node.spans(0)).end))
    ...
    node.spans.flatMap {
      span =>
        val spanInDependency = dependencies.annotationSpan((graph.spans(span).start, graph.spans(span).end))
        IndexedSeq(spanInDependency._1, spanInDependency._2)
    }
  }
