...
object ExtractConceptTable {
  ...
  def parseOptions(map: OptionMap, list: List[String]): OptionMap = {
    ...
    list match {
      ...
      case "-f" :: value :: l => parseOptions(map + ('file -> value), l)
  ...
  def main(args: Array[String]) {
    ...
    val fileOption = options.get('file)
    logger(0, "stage1Features = " + stage1Features)

    val conceptTable: m.Map[String, List[PhraseConceptPair]] = if (fileOption.isDefined) {
      extract(splitOnNewline(Source.fromFile(fileOption.get).getLines).toArray, stage1Features, maxCount)
    } else {
      extract(splitOnNewline(Source.stdin.getLines).toArray, stage1Features, maxCount)
    }
